...
-- Modified to support context.user.moderator format used by Jitsi
...
log('info', 'Loaded token moderation plugin (modified for context.user.moderator)');
        ...
        if origin.auth_token then
                ...
                if dotFirst then
                        ...
                        if dotSecond then
                                ...
                                -- Check moderator field in multiple locations
                                local dominated_by_token = false;
                                
                                -- Check context.user.moderator (Jitsi standard format)
                                if body.context and body.context.user and body.context.user.moderator == true then
                                        dominated_by_token = true;
                                        log('info', 'User %s is moderator (from context.user.moderator)', jid);
                                -- Also check root level moderator for backward compatibility
                                elseif body["moderator"] == true then
                                        dominated_by_token = true;
                                        log('info', 'User %s is moderator (from root moderator field)', jid);
                                end
                                
                                -- If user is a moderator or an admin, set their affiliation to be an owner
                                if dominated_by_token or is_admin(jid) then
                                        log('info', 'Setting %s as owner', jid);
                                        room:set_affiliation("token_plugin", jid, "owner");
                                else
                                        log('info', 'Setting %s as member (not moderator)', jid);
